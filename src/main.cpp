...
#if defined(NDEBUG)
# error "Tsunamicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x95e6bbd9b9bf4f72d561c5bb8e70677a4b55cd40762d6f38e3f1ef0a50470862");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tsunamicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tsunamicoin Signed Message:\n";
...
{
    // Tsunamicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 25000); // Tsunamicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Tsunamicoin: 3 days
static const int64 nTargetSpacing = 8 * 60; // Tsunamicoin: 8 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tsunamicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tsunamicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9c7de6e38cec670830688322adcb8ed2a5951d0c706046874eed7d734e41a7e3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "War starts on minecraft server 06/13/2022";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040ab843e0ffbf16ce08531c7f5dcf6ea530b54b7b9c1c2dd4ac699cffae5426bae08180b67b3c4ddf0afcd0bb729231efe5646b32606a42789e2c66a991253a48") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1655160114;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088173233;

        if (fTestNet)
        {
            block.nTime    = 1655160095;
            block.nNonce   = 386033769;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5a902d6a41a190c6ea9a3fc2d99b2410a199e28d69a2d8243b56c6304a0acd2d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xce, 0xb7, 0xd2 }; // Tsunamicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TsunamicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TsunamicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TsunamicoinMiner(CWallet *pwallet)
{
    printf("TsunamicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tsunamicoin-miner");
    ...
    try { loop {
        ...
        printf("Running TsunamicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TsunamicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TsunamicoinMiner, pwallet));
}
